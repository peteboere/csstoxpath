import {tokenStreams, decorateToken, translateCaseMap,
    arrayFilterUnique, applyCustomPsuedos, quotedString} from './helpers.js';

/** @typedef {import('./_.d.ts').Token} Token */
/** @typedef {import('./_.d.ts').PseudoDefinitions} PseudoDefinitions */

/**
 * Convert CSS selector to XPath expression
 * @param {string} selector
 * @param {{pseudos?: PseudoDefinitions}} [options={}]
 */
export function cssToXPath(selector, {pseudos}={}) {

    /** @type {Token[][]} */
    const streams = tokenStreams(selector, pseudos);
    const expressions = [];
...
/**
 * Convert CSS selector to XPath sub-expression.
 */
cssToXPath.subExpression = subExpression;

/**
 * Interpolate custom pseudo classes.
 */
cssToXPath.applyCustomPsuedos = applyCustomPsuedos;

/**
 * Normalise string for CSS selector expressions.
 */
cssToXPath.quotedString = quotedString;

/**
 * @param {string} selector
 * @param {{pseudos?: PseudoDefinitions}} options
 */
function subExpression(selector, {pseudos}={}) {
    ...
    return resolveSubExpression(streams, {operator: 'or'});
}
        ...
        switch (token.type) {
            ...
            default: {
                ...
                if (token.isPseudoNot) {
                    filters.push(`not(${resolveSubExpression(data, {operator: 'or'})})`);
                }
                else if (token.isPseudoAny) {
                    filters.push(subExpression(data));
                }
...
function resolveSubExpression(tokens, options={}) {
    ...
    if (token.isTag) {
        ...
        switch (name) {
            ...
            case 'last-of-type':
                ...
                break;
            case 'not':
                elements.push(`not(${resolveSubExpression(data, {operator: 'or'})})`);
                break;
